...
// read nbuf bytes from stdin to buf
int
...
gettoken(char **ps, char *es, char **q, char **eq)
{
  ...
  s = *ps;
  while(s < es && strchr(whitespace, *s))   // if s == one of "\t\r\n\v", skip it, until it's not.
    s++;
  if(q)             // if q is pass to this function, set it pointer to s
    *q = s;
  ret = *s;         // ret = now s's content
  switch(*s){       // swicth *s: 0, '|', '<', '>', s++; other view set ret = 'a'
  case 0:
    break;
  case '|':
  case '<':
  ...
  default:
    ...
    while(s < es && !strchr(whitespace, *s) && !strchr(symbols, *s)) 
      s++;      // check s is one of whitespace or symbols, if not, s++
    break;      // until next token
  }
  if(eq)
    *eq = s;
...
parsecmd(char *s)
{
  ...
  es = s + strlen(s);       // stirng end pointer
  cmd = parseline(&s, es);
  peek(&s, es, "");
  if(s != es){
...
// call parsepipe, return what it return
struct cmd*
